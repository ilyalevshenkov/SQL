...
/*ДЗ 4. SQL – работа с несколькими таблицами
Задача 1. Подсчитать общее количество лайков, которые получили пользователи младше 12 лет */

/*По факту ошибка - получились пользователи, КОТОРЫЕ лайкали сами, а не те КОГО лайкали.*/
...
/* Var 2.  id пользователей, котороые удовлетворяют условиям*/
-- Вариант решения через JOIN, но общую сумму не выводит
select profiles.user_id, birthday, sum(likes.media_id )
from profiles
join likes
on profiles.user_id = likes.user_id
where birthday > '2011-04-24'
group by profiles.user_id;

-- Вариант решения через вложенный список

select sum(media_id) as 'Total likes'
from likes
where user_id in (
	select user_id 
	from profiles
...
SELECT CASE (gender)
	WHEN 'm' THEN 'Man'
	WHEN 'f' THEN 'Woman'
    END AS 'More likes did:', COUNT(*) as 'Total likes'
FROM profiles p 
JOIN likes l 
...
SELECT DISTINCT CONCAT(firstname, ' ', lastname, ' (id: ', (id), ')') AS 'Did not messages'
FROM users
WHERE NOT EXISTS (
	SELECT from_user_id
	FROM messages
...
SELECT from_user_id AS 'sendler_id', 
	(SELECT CONCAT(firstname,' ', lastname) 
    FROM users 
    WHERE id = messages.from_user_id) AS 'lastname, firstname', COUNT(*) AS `Total_messages`
FROM messages 
